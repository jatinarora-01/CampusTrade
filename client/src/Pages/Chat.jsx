...
function Chat() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(usersRef, (querySnapshot) => {
      const usersPromises = querySnapshot.docs.map(async (doc) => {
        ...
        const imgUrl = await getDownloadURL(imgRef).catch(
          () => "defaultImageUrl"
        );
        return {
          id: doc.id,
  ...
  const handleFileChange = async (event) => {
    ...
    const fileRef = ref(
      storage,
      `chatFiles/${currentUser.uid}_${new Date().getTime()}_${file.name}`
    );
        ...
        <div className="w-1/3 video-call-container overflow-auto">
          <>
            ...
            <form
              ...
              className="w-full p-2 border-t border-gray-300 flex flex-row"
            >
              <input
                type="text"
                value={newMessage}
          ...
          {!selectedUser && (
            <>
              {users.map((user) => (
                <div
                  key={user.id}
                  className="p-4 flex items-center border-b border-gray-300 cursor-pointer"
